...
function ReceiptPage() {
  ...
  const query = trpc.payments.getIntent.useQuery({ id: intentId }, { enabled: Boolean(intentId) });
  ...
  const amount = (Number(intent.amountAtomic) / 1_000_000).toLocaleString(undefined, { minimumFractionDigits: 2 });
  const isSucceeded = intent.status === "succeeded";
  return <div className="checkout-shell"><div className="checkout-brand"><Mark /><span>druto</span><span className="checkout-test"><span className="live-dot" /> Buyer receipt</span></div><main className="checkout-main"><div className="checkout-intro"><span className="eyebrow"><span className="eyebrow-line" /> Payment receipt</span><h1>Payment <em>{isSucceeded ? "verified." : "awaiting verification."}</em></h1><p>{isSucceeded ? "Your USDC payment was recorded on Arc Testnet and accepted by the merchant." : "This receipt will update after Druto verifies the Arc Testnet transaction."}</p></div><div className="checkout-card receipt-card"><div className={isSucceeded ? "success-mark" : "submitted-orbit"}>{isSucceeded ? <Check size={26} /> : <RefreshCw size={24} />}</div><span className="eyebrow">{isSucceeded ? "Payment final" : "Payment pending"}</span><h2>${amount} <small>USDC</small></h2><div className="receipt-lines"><DetailLine label="Payment Intent" value={intent.id} mono /><DetailLine label="Order" value={intent.externalOrderId} /><DetailLine label="Item" value={intent.itemName} /><DetailLine label="Buyer" value={intent.buyerLabel ?? "Wallet buyer"} /><DetailLine label="Network" value="Arc Testnet · USDC" /><DetailLine label="Merchant" value={intent.merchantAddress} mono /><DetailLine label="Transaction" value={intent.transactionHash ?? "Pending"} mono /></div>{intent.transactionHash && <a className="button button-primary full-width" href={`https://testnet.arcscan.app/tx/${intent.transactionHash}`} target="_blank" rel="noreferrer"><ExternalLink size={15} /> View verified transaction</a>}{intent.returnUrl && <button className="button button-quiet full-width" onClick={() => window.location.href = intent.returnUrl!}>Return to marketplace</button>}<div className="checkout-footer"><span>Receipt generated by <strong>druto</strong></span><span><LockKeyhole size={12} /> {isSucceeded ? "Verified onchain" : "Awaiting Arc verification"}</span></div></div></main></div>;
}
...
function CheckoutPage() {
  ...
  const prepareTransfer = trpc.payments.prepareTransfer.useMutation();
  const verifyTransfer = trpc.payments.verifyTransfer.useMutation();
  const intent = intentQuery.data;
  const isRealIntent = Boolean(intent);
  useEffect(() => {
    if (!intentId) return;
    QRCode.toDataURL(window.location.href, { width: 220, margin: 1 }).then(setQrCode).catch(() => setQrCode(null));
  }, [intentId]);
  const amountDisplay = intent ? (Number(intent.amountAtomic) / 1_000_000).toLocaleString(undefined, { minimumFractionDigits: 2 }) : "1,240.00";
  const connectAndPay = async () => {
    setError(null);
    if (!isRealIntent || !intent) {
      setState("submitted");
      return;
    }
    const ethereum = (window as Window & { ethereum?: { request: (args: { method: string; params?: unknown[] }) => Promise<unknown> } }).ethereum;
    if (!ethereum) {
      setError("Install MetaMask, Rabby, or another injected EVM wallet to continue.");
      return;
    }
    try {
      ...
      if (Number.parseInt(chainId, 16) !== 5042002) {
        await ethereum.request({ method: "wallet_addEthereumChain", params: [{ chainId: "0x4cef52", chainName: "Arc Testnet", nativeCurrency: { name: "USDC", symbol: "USDC", decimals: 18 }, rpcUrls: ["https://rpc.testnet.arc.io"], blockExplorerUrls: ["https://testnet.arcscan.app"] }] });
      }
      setWalletAddress(buyer);
      const request = await prepareTransfer.mutateAsync({ id: intent.id, buyerAddress: buyer });
      const data = encodeFunctionData({ abi: request.abi, functionName: request.functionName, args: request.args });
      const hash = await ethereum.request({ method: "eth_sendTransaction", params: [{ from: buyer, to: request.tokenAddress, data, value: "0x0" }] }) as string;
      setTransactionHash(hash);
      setState("submitted");
      const verified = await verifyTransfer.mutateAsync({ id: intent.id, transactionHash: hash });
      if (verified.status === "succeeded") setState("final");
    } catch (cause) {
      setError(cause instanceof Error ? cause.message : "Wallet payment could not be completed.");
      setState("ready");
    }
  };
  return <div className="checkout-shell"><div className="checkout-brand"><Mark /><span>druto</span><span className="checkout-test"><span className="live-dot" /> Arc Testnet</span></div><main className="checkout-main"><div className="checkout-intro"><span className="eyebrow"><span className="eyebrow-line" /> Hosted Checkout</span><h1>Pay <em>Northstar AI.</em></h1><p>One USDC transfer. Final on Arc, recorded in Druto.</p><div className="checkout-trust"><span><ShieldCheck size={15} /> Verified merchant</span><span><Zap size={15} /> Fast finality</span></div></div><div className="checkout-card"><div className="checkout-card-head"><span>Order #{intent?.externalOrderId ?? "NS-1842"}</span><span className="checkout-expiry"><Timer size={14} /> {intent ? `Expires ${new Date(intent.expiresAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Demo intent"}</span></div><div className="checkout-amount"><span>Total due</span><strong>${amountDisplay} <small>USDC</small></strong></div><div className="checkout-network"><span className="network-symbol">A</span><span><strong>USDC on Arc</strong><small>Arc Testnet · ERC-20 · chain 5042002</small></span><BadgeCheck size={17} /></div>{state === "ready" && <><div className="wallet-placeholder"><div className="wallet-ring"><WalletCards size={26} /></div><strong>{walletAddress ? "Wallet connected" : "Connect a wallet to continue"}</strong><span>Your wallet will only be asked to approve this exact amount to the verified merchant wallet.</span><button className="button button-primary full-width" onClick={connectAndPay} disabled={prepareTransfer.isPending || verifyTransfer.isPending}><WalletCards size={16} /> {prepareTransfer.isPending ? "Preparing transfer…" : walletAddress ? "Pay with wallet" : "Connect wallet"}</button></div><div className="checkout-note"><LockKeyhole size={13} /> Review recipient and amount in your wallet before signing.</div><div className="qr-panel"><div>{qrCode ? <img src={qrCode} alt="QR code for this Druto checkout" /> : <span className="qr-placeholder">Generating QR…</span>}</div><span><strong>Pay from your phone</strong><small>Scan to open this exact Druto checkout link.</small></span></div></>}{state === "submitted" && <div className="wallet-placeholder state-submitted"><div className="submitted-orbit"><RefreshCw size={25} /></div><strong>Transaction submitted</strong><span>{transactionHash ? "Arc is verifying the exact USDC transfer." : "Demo transaction submitted."}</span>{transactionHash && <a className="mono-id" href={`https://testnet.arcscan.app/tx/${transactionHash}`} target="_blank" rel="noreferrer">View transaction on ArcScan</a>}<div className="progress-line"><i /></div></div>}{state === "final" && <div className="wallet-placeholder state-final"><div className="success-mark"><Check size={26} /></div><strong>Payment final</strong><span>Your payment is verified and Northstar AI can fulfill your order.</span><div className="final-ref"><span>Transaction</span><strong className="mono-id">{transactionHash ?? "demo_0x7a2b…d91c"} <Copy size={13} /></strong></div><button className="button button-primary full-width" onClick={() => intent ? window.location.href = `/receipt/${intent.id}` : toast.success("Payment confirmed for the marketplace order.")}>View buyer receipt</button>{intent?.returnUrl && <button className="button button-quiet full-width" onClick={() => window.location.href = intent.returnUrl!}>Return to marketplace</button>}</div>}{error && <div className="checkout-note" role="alert"><AlertTriangle size={13} /> {error}</div>}<div className="checkout-footer"><span>Powered by <strong>druto</strong></span><span><LockKeyhole size={12} /> Arc Testnet · USDC only</span></div></div></main></div>;
}

function MarketplacePage() { const createIntent = trpc.payments.createIntent.useMutation(); const [demoComplete, setDemoComplete] = useState(false); const buy = async () => { try { const result = await createIntent.mutateAsync({ externalOrderId: "NS-1842", idempotencyKey: "marketplace-ns-1842", itemName: "Northstar API Pro — annual access", buyerLabel: "Hackathon buyer", returnUrl: "/marketplace", amount: "1.00" }); window.location.href = result.checkoutUrl; } catch (error) { toast.error(error instanceof Error ? error.message : "Unable to open Druto checkout"); } }; return <div className="marketplace-shell"><header className="marketplace-nav"><div className="brand-lockup"><Mark /><span>northstar marketplace</span></div><span className="checkout-test"><span className="live-dot" /> Arc Testnet demo</span></header><main className="marketplace-main"><div className="marketplace-breadcrumb">Marketplace / Developer tools / Annual access</div><section className="marketplace-product card"><div className="product-art"><img src={heroVisual} alt="Abstract product network" /></div><div className="product-copy"><span className="eyebrow">Digital access · Order NS-1842</span><h1>Northstar API Pro</h1><p>Ship reliable data products with a production-shaped API workspace, usage insights, and team controls.</p><div className="product-meta"><span><Check size={15} /> Instant access</span><span><ShieldCheck size={15} /> Verified seller</span><span><Zap size={15} /> USDC on Arc</span></div><div className="product-price"><span>Total</span><strong>$1.00 <small>USDC</small></strong></div><button className="button button-primary product-buy" onClick={buy} disabled={createIntent.isPending}><WalletCards size={16} /> {createIntent.isPending ? "Opening Druto…" : "Pay with Druto"}</button><button className="text-button marketplace-demo" onClick={() => { setDemoComplete(true); toast.success("Demo order paid — no blockchain transaction was sent."); }}>Use demo fallback instead</button>{demoComplete && <div className="marketplace-success"><Check size={16} /><span>Demo order complete. Open Seller Dashboard or Buyer Receipt for the presentation walkthrough.</span></div>}<small className="marketplace-note">Real path: Druto creates a Payment Intent, then verifies your wallet transfer on Arc Testnet. Demo fallback never moves funds.</small></div></section><div className="marketplace-trust"><span><LockKeyhole size={15} /> Exact amount shown before wallet approval</span><span><ReceiptText size={15} /> Buyer receipt after verification</span><span><Activity size={15} /> Seller dashboard updates from verified activity</span></div></main></div>; }


export default function Home() { const [location] = useLocation(); if (location.startsWith("/marketplace")) return <MarketplacePage />; if (location.startsWith("/receipt")) return <ReceiptPage />; if (location.startsWith("/checkout")) return <CheckoutPage />; const [active, setActive] = useState(location.startsWith("/payments") ? "Payments" : "Overview"); const [collapsed, setCollapsed] = useState(false); const [showCreate, setShowCreate] = useState(false); const title = active === "Overview" ? "Overview" : active; const create = () => setShowCreate(true); return <div className="app-shell"><Sidebar active={active} setActive={setActive} collapsed={collapsed} setCollapsed={setCollapsed} /><main className="main-shell"><Topbar title={title} onCreate={create} />{active === "Overview" ? <Overview setActive={setActive} onCreate={create} /> : <PagePanel active={active} onCreate={create} />}</main>{showCreate && <CreatePayment close={() => setShowCreate(false)} />}</div>; }

function CreatePayment({ close }: { close: () => void }) {
  const [amount, setAmount] = useState("1.00");
  const [orderId, setOrderId] = useState("NS-1842");
  const [itemName, setItemName] = useState("Northstar API Pro — annual access");
