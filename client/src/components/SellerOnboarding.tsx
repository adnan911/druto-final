...
import { Check, Clipboard, ExternalLink, KeyRound, ShieldCheck, Webhook } from "lucide-react";
...
export default function SellerOnboarding() {
  ...
  const reconcile = async () => {
    if (!/^0x[a-fA-F0-9]{64}$/.test(legacyTransactionHash.trim()) || !marketplaceId.trim() || !sellerId.trim()) {
      toast.error("Enter the exact 64-byte transaction hash and seller identifiers");
      return;
    }
    try {
      ...
      toast.success("Legacy payment linked to the seller account");
    } catch (error) {
  ...
  return <div className="page-content seller-onboarding-page">
    ...
    <div className="onboarding-progress"><div className="onboarding-progress-step"><span>01</span><strong>Register seller</strong><small>Identity + payment destination</small></div><div className="onboarding-progress-step"><span>02</span><strong>Provision credentials</strong><small>API key + webhook secret</small></div><div className="onboarding-progress-step"><span>03</span><strong>Review account</strong><small>Operator activation</small></div></div>
    {merchantStatus && <div className="card onboarding-success"><Check size={18} /><div><strong>Seller account created as {merchantStatus}</strong><p>An authorized operator can activate this account after reviewing the configured payment destination.</p></div></div>}
    ...
    <div className="card reconciliation-card"><div className="card-heading"><div><span className="eyebrow">Legacy payment repair</span><h3>Link an earlier unscoped payment</h3></div><ShieldCheck size={18} /></div><p>Use this only for a succeeded payment created before seller routing was enabled. Druto will link it only when the active seller identifiers and payment destination exactly match the verified transaction.</p><div className="reconciliation-fields"><input value={marketplaceId} onChange={event => setMarketplaceId(event.target.value)} placeholder="Marketplace ID, e.g. luvre-franc" aria-label="Reconciliation marketplace ID" /><input value={sellerId} onChange={event => setSellerId(event.target.value)} placeholder="Seller ID, e.g. luvre-main" aria-label="Reconciliation seller ID" /><input className="field-wide" value={legacyTransactionHash} onChange={event => setLegacyTransactionHash(event.target.value)} placeholder="Arc transaction hash" spellCheck={false} aria-label="Legacy transaction hash" /></div><button className="button button-quiet" type="button" onClick={() => void reconcile()} disabled={reconcileLegacy.isPending}>{reconcileLegacy.isPending ? "Checking payment destination…" : "Reconcile verified transaction"}</button>{reconcileMessage && <div className="reconciliation-success"><Check size={15} /> {reconcileMessage}</div>}</div>
    <form className="card onboarding-form" onSubmit={submit}><div className="card-heading"><div><span className="eyebrow">Step 1 · Seller identity</span><h3>Connect your website to Druto</h3></div><KeyRound size={18} /></div><p>Use a stable seller ID from your website and configure the payment destination that will receive settlement when wallet payments are re-enabled.</p><div className="onboarding-fields"><label>Marketplace ID<input value={marketplaceId} onChange={event => setMarketplaceId(event.target.value)} placeholder="dashda" maxLength={128} /></label><label>Seller ID<input value={sellerId} onChange={event => setSellerId(event.target.value)} placeholder="seller_123" maxLength={128} /></label><label className="field-wide">Display name<input value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="Dashda Clothing" maxLength={255} /></label><label className="field-wide">Payment destination<input value={receivingAddress} onChange={event => setReceivingAddress(event.target.value)} placeholder="0x..." maxLength={42} spellCheck={false} /></label><label className="field-wide">Webhook URL<input value={webhookUrl} onChange={event => setWebhookUrl(event.target.value)} placeholder="https://dashda.com/api/webhooks/druto" type="url" maxLength={2048} /></label></div><button className="button button-primary" type="submit" disabled={busy}>{busy ? "Provisioning seller workspace…" : "Create seller workspace"} <ExternalLink size={14} /></button></form>
    <div className="onboarding-boundaries"><div><KeyRound size={17} /><span><strong>API key</strong><small>Use only from your website backend to create Payment Intents.</small></span></div><div><Webhook size={17} /><span><strong>Webhook secret</strong><small>Use only on your backend to verify signed payment.verified events.</small></span></div><div><ShieldCheck size={17} /><span><strong>Operator review</strong><small>Account activation is controlled by the Druto operator in this temporary mode.</small></span></div></div>
  </div>;
}
